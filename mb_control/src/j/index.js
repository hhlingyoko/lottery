...
(function() {
  ...
  var oPage = {
    ...
    initPageInfo: function() {
      ...
      let curNum = localStorage.getItem("curNum")
      curStarted =  localStorage.getItem("curStarted")
      
      let idx = 0;
      ui.$inputNum.val(curNum);
      if(id){
        idx = prizeInfo.findIndex( item => {
          return item.id == id
        })
      }
    ...
    initHistory: function() {
      ...
      $.ajax({
        ...
        let h = ''
        aHistoryList = msg
        aHistoryList.forEach((item, index)=>{
    ...
    startLottery: function () {
      var self = this   
      let curLotteryNum = Number(ui.$inputNum.val())
      let usedNum = Number(ui.$usedNum.html())
    ...
    reload: function (jqXHR) {
      console.log(jqXHR)
      var self = this;
      let xErrorCode = jqXHR.getResponseHeader('X-Error-Code')
      ...
      if(xErrorCode == 302) {
        ...
        let text = jqXHR.responseText? jqXHR.responseText:'操作失败'
