...
$(function () {
  ...
  var oPage = {
    ...
    // 初始化图片信息
    loadPersonImg: function loadPersonImg(bFirstInit, rotateSpeed) {
      // var self = this; 

      // 随机mock头像信息
      var aImg = [];
      for (var i = 0, _len = 300; i < _len; i++) {
      ...
      // 处理图片
      var len = Number(aImg.length) > nMaxPhoto ? nMaxPhoto : aImg.length;
      for (var _i = 0; _i < len; _i++) {
        personArray.push({ image: aImg[_i] });
      }
      for (var _i2 = 0; _i2 < personArray.length; _i2++) {
        table[_i2] = new Object();
        if (_i2 < personArray.length) {
          table[_i2] = personArray[_i2];
          table[_i2].src = personArray[_i2].thumb_image;
        }
        table[_i2].p_x = _i2 % 20 + 1;
        table[_i2].p_y = Math.floor(_i2 / 20) + 1;
      }
      this.initSphere();
      if (bFirstInit) {
    ...
    // },    
    // 投屏奖品信息
    throwScreen: function throwScreen(data) {
      var self = this;
    ...
    // 开始快速旋转
    startLottery: function startLottery(data) {
      var self = this;
      // 奖品信息更改
      $('.js-prize-name').html(data.award.name);
      $('.js-prize-num').html(data.num);
    ...
    // 停止旋转
    stopLottery: function stopLottery(data) {
      var self = this;
      ...
      // 中奖信息
      var html = '';
      var len = data.award_users.length;
      var type = data.award_users.length > 24 ? 'size--small' : 'size--big';
      data.award_users.forEach(function (item, idx) {
    ...
    initSphere: function initSphere() {
      var self = this;
    ...
    transform: function transform(targets, duration, type) {
      ...
      new TWEEN.Tween(this).to({}, duration * 2).onUpdate(self.render).start();
    },
    ...
    animate: function animate(rotateSpeed) {
      var self = this;
